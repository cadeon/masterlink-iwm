    ...
    var ActionEditPopup = Class.extend( AbstractCrudPopup, {
        ...
        show: function(theId){
            if(theId != null && theId != -1){
                ...
                _this = this;
                TaskActions.getItem(function(response){_this.populate(response);}, theId);
            } else {
                //if no locator id is passed in that means it is an add. make a blank form.
                $("popupTitle").innerHTML= "Add Action Data";
                this.theData.id=-1;
                this.theData.taskId=<%= request.getParameter("taskId") %>;
            }
            thePopupManager.showPopup('objects-tasks-actions_edit');
            $('verb').focus();
        },
        ...
        if(this.validate()){
            ...
            var _this = this;
            //TaskActions.saveItem(function(message){_this.showCallBackMessage(message)}, this.theData);
            this.persist(this.theData);
        }}
    });

callOnLoad(function(){
    theActionEditPopup = new ActionEditPopup("objects-tasks-actions_edit", "ActionForm", dataTable);
});
</script>
...
--%>
<style type="text/css">
div#objects-tasks-actions_edit input.textInput {
    width: 175px;
}
div#objects-tasks-actions_edit {
    width: 270px;
    left: -135px;
}
...
<div class="popup" id="objects-tasks-actions_edit">
