...
Worker.id=<%=request.getParameter("id")%>;
var TimeSpecsDataTableConfig = Class.extend( DataTableConfig, {
    ...
    getJobCount: function(item) {
        <%/*here item.id is workschedid*/%>
        var theIcon = '<div class="icon" onclick="document.location.href=\'WorkerCalendarJobs.do?wsId=' + item.id + '&id='+Worker.id+'\'">';
    ...
    getFormaters: function(){return [this.getDelete, this.getDate, this.getFullLocator,
            this.getHours, this.getStatus, this.getJobCount];}
});
...
function endShift(){
    ...
    if(ids.length==1)
    	showShift(<%=request.getParameter("id")%>, ids[0]);
    else
        alert("You need to select only one item to end its shift!")
}
...
function init() {
    //setup the dataTable
    theConfig = new TimeSpecsDataTableConfig(TimeSpecs, ["day","fullLocator","jobCount","statusDesc"]);
    ...
    locatorFilter = new LocatorChain("locatorFilter",
            ...
            false, true, 1);
    locatorFilter.populateChain(<%=cr.getLocatorId()%>);
    ...
    /*SessionUtil.getCurrentLocator({
        ...
        async:false});*/
    //DWRUtil.setValue($("personId"),Worker.id);
    workerCalendarTable.update();   
}
callOnLoad(init);
</script>
...
<input type="button" class="button" value="Add to Calendar" onclick="showCalendarSpecs(<%=request.getParameter("id")%>,locatorFilter.currentSelectedId);"/>
<input type="button" class="button" value="Delete Selected" onclick="getDeleted();"/>
<input type="button" class="button" value="End Shift Selected" onclick="endShift();"/>
