 ...
 * Date: Oct 12, 2006
 */
public class JobScheduleMaint extends BaseAction {
    private static final Logger logger = Logger.getLogger(JobScheduleMaint.class);
    ...
    public ActionForward executeLogic(ActionMapping mapping,
        ...
        try {
            ...
            if(jvo.getOrganizationId()!=null){
                Organization org = (Organization)psf.get(Organization.class, jvo.getOrganizationId());
            ...
            context.add("skillTypeId", SkillTypeRef.getLabel(jvo.getSkillTypeId()));
            context.add("skillLevel", SkillLevelRef.getLabel(jvo.getSkillLevelId()));
            if(jvo.getNumberOfWorkers()!=null){
                context.add("numberOfWorkers", String.valueOf(jvo.getNumberOfWorkers()));
            }
            context.add("status", JobStatusRef.getLabel(jvo.getStatusId()));
            context.add("estTime", ConvertUtils.formatDuration(jvo.getEstTime()));
