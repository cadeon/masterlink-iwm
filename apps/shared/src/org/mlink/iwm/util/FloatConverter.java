...
public class FloatConverter implements Converter {
    ...
    public Object convert(Class type, Object value) {
        ...
        if (value == null) {
            ...
            if(str.length()==0){
                rtn = CopyUtils.isBaseVOTarget()?CopyUtils.NullAliasValues.get(CopyUtils.NullAlias.Float):null;
            }else{
                // try to parse w/ decimal formatter
                ...
                try {
                    rtn = ((decFmt.parse((String) value)).floatValue());
                } catch (ParseException e) {
                    throw new ConversionException("DoubleConverter: " + e.getMessage());
                }
            }
        } else {
            rtn = (new Float(value.toString()));
        }
        return rtn;
    }
